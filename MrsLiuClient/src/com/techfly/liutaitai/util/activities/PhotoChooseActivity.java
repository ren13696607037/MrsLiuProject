...
import com.techfly.liutaitai.util.adapter.FolderListAdapter;
import com.techfly.liutaitai.util.adapter.PhotoListAdapter;
import com.techfly.liutaitai.util.view.CropImageView.CropMode;
import com.techfly.liutaitai.util.view.FloatingActionButton;
...
public class PhotoChooseActivity extends PhotoBaseActivity implements View.OnClickListener, AdapterView.OnItemClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mCropMode = (CropMode) getIntent().getSerializableExtra(GalleryHelper.CROP_MODE);
        mType = getIntent().getIntExtra(GalleryHelper.APPLY_TYPE, -1);
    ...
    /**
     * 解决�?5.0手机上刷新Gallery问题，从startActivityForResult回到Activity把数据添加到集合中然后理解跳转到下一个页面，
     * adapter的getCount与list.size不一致，�?以我这里用了延迟刷新数据
     * @param photoInfo
     */
    ...
    protected void takeResult(PhotoInfo photoInfo) {
        ...
        if ( mPickMode == GalleryHelper.SINGLE_IMAGE ) { //单�??
            if ( mCrop ) {//裁剪
                mHasRefreshGallery = true;
                toPhotoCrop(photoInfo, mCropMode, mType);
            } else {
    ...
    private void photoItemClick(View view, int position) {
        ...
        if (mPickMode == GalleryHelper.SINGLE_IMAGE) {
            if (mCrop) {
                mHasRefreshGallery = true;
                toPhotoCrop(info, mCropMode, mType);
            } else {
                resultSingle(info);
            }
            return;
